...
router
  ...
  .delete((req, res) => {
    ...
    Room.destroy({ where: { id } })
      .then((deletedRoom) => (deletedRoom ? res.json(id) : res.status(404).json(deletedRoom)))
      .catch((error) => res.status(500).json({ error: error.message }));
  })
  .put((req, res) => {
    const { id } = req.params;
...
router.route('/room')
  .post((req, res) => {
    ...
    Room.create({
      ...
      .then((newRoom) => (newRoom ? res.status(200).json({ id: newRoom.id, name: newRoom.name, description: newRoom.description }) : res.status(409).json({ message: `failed to create new room with name=${name}` })), // 409 conflict
      )
      .catch((error) => res.status(500).json({ error: error.message }));
  });
